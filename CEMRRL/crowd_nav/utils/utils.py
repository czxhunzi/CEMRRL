...
def rotate_s(state):
    ...
    # da=torch.min(da).repeat(7).view(7,1)
...
def obs2state(robot, obs):
    state = rotate_s(JointState(robot.get_full_state(), obs))
    return state
...
class ReplayBuffer2:
    ...
    def push(self, states, actions, rewards, next_states, dones):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        if len(self.buffer) >= self.capacity and self.flag:
            print('Replay experience pool is full')
            self.flag = False
    ...
    def sample(self, batch_size):
        ...
        def transpose_and_stack(data_list):
            num_robots = len(data_list[0])  # 每条数据有几个机器人
            batch_size = len(data_list)  # 有多少条数据（batch size）

            return [
                torch.stack([
                    torch.tensor(data_list[b][i], dtype=torch.float32) if not isinstance(data_list[b][i], torch.Tensor)
                    else data_list[b][i]
                    for b in range(batch_size)  # 对 batch 中的每条数据遍历
                ]) for i in range(num_robots)  # 每个机器人一组
            ]

        states = [s.to(self.device) for s in transpose_and_stack(states_batch)]         # list[num_robots][batch_size, state_dim]
        actions = [a.to(self.device) for a in transpose_and_stack(actions_batch)]       # list[num_robots][batch_size, action_dim]
        rewards = [r.to(self.device) for r in transpose_and_stack(rewards_batch)]       # list[num_robots][batch_size]
        next_states = [ns.to(self.device) for ns in transpose_and_stack(next_states_batch)]

        dones = torch.tensor(dones_batch, dtype=torch.float32).to(self.device).unsqueeze(1)
